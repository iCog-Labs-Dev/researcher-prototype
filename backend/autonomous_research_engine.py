...
class AutonomousResearcher:
    ...
    def __init__(self, user_manager: UserManager):
        ...
        logger.info(f"🔬 LangGraph Autonomous Researcher initialized - Enabled: {self.enabled}, Interval: {config.RESEARCH_INTERVAL_HOURS}h")
    ...
    async def _conduct_research_cycle(self):
        ...
        try:
                ...
                try:
                    ...
                    for topic in topics_to_research:
                        try:
                            ...
                            logger.error(f"🔬 Error researching topic {topic.get('topic_name')} for user {user_id}: {str(e)}")
                            continue
                    
                    # Cleanup old findings for this user
                    self.user_manager.cleanup_old_research_findings(
                        user_id, 
                        config.RESEARCH_FINDINGS_RETENTION_DAYS
                    )
            ...
            logger.info(f"🔬 LangGraph research cycle completed: {total_topics_researched} topics researched, {total_findings_stored} findings stored")
    ...
    async def _research_topic_with_langgraph(self, user_id: str, topic: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            research_state = {
                ...
                "session_id": None,  # Will be generated by research_initializer_node
                "memory_context": None
            }
